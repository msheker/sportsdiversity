// This file should work with json (see revisedfunction makeResponsive() {
...
// json from the API endpoint (created by the app.py file in @appget_coaches)
  // this will grab the json dat that is returned from the API and use it to make something
   d3.json("http://127.0.0.1:5000/get_info")
     .then(function (nbaData) {
       console.log(nbaData)
    var parseTime = d3.timeParse("%Y");
       nbaData.forEach(function (data) {
         data[0] = parseTime(data.date); // the JSON is just values, no keys
         data[1] = +data.White;
         data[2] = +data.African;
         data[3] = +data.Latino;
       });
